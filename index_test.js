...
// Define network path for Excel file
const excelFilePath =
  "\\\\192.168.1.30\\Dokumentai\\PPS\\2. Išoriniai\\Hidrologinės prognozės\\@ Prognozės\\Test\\Dienos situacija_test.xlsx";
...
const runBasedOnTimeWindow = async () => {
  await setDateInExcelSheet(excelFilePath, "Paros pokytis", "B1"); // Set the current date in cell B1
  ...
  if (
    ...
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.readFile(excelFilePath); // Read Excel file
    ...
    await updateExcelFromCSV(
      ...
      excelFilePath, // Excel file to update
